...
`tools/XY7320Host/` 是当前项目的 Qt6 + QML + C++ 串口升级上位机，用于配合 `XY_bootloadr` 完成 U 盘 APP 固件发送、串口调试和升级日志查看。
...
```text
1. 在“设置 → 参数配置”中选择串口、波特率并打开全局设备串口。
2. 在“固件升级”页从可移动盘根目录选择 APP `.bin`（本地浏览可作为兜底）。
3. 点击开始升级；升级过程中串口由升级流程独占，串口调试发送会自动禁用。
...
- 串口选择、波特率和开关只放在“设置 → 参数配置 → 设备连接”卡片；升级页和串口调试页仅显示并复用该全局串口状态。
...
- “固件升级”页内集成 U 盘固件包列表和升级日志；不再提供独立 U 盘读取、升级日志、固件信息、协议参数或发送设置页面/卡片。协议与发送参数以日志顶部的灰色摘要显示。
